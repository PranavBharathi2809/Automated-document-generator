...
def map_record(record: dict):
    ...
    for template_key, excel_key in FIELD_MAPPING.items():
        ...
        if key == "document number":
            mapped["DocNo"] = str(raw_value).strip() if raw_value else "DOC001"
            continue            
        # ==============================
        # PROPOSED CTC
        # ==============================
        ...
        # ==============================
        # REVISED CTC (FIXED INDENTATION)
        # ==============================
        if key == "revised ctc":
            if raw_value in ["", None]:
                mapped["Revised_CTC"] = ""
                mapped["Revised_CTC_Words"] = ""
            else:
                try:
                    formatted, words = convert_to_words_indian(raw_value)
                    mapped["Revised_CTC"] = formatted
                    mapped["Revised_CTC_Words"] = words
                except Exception as e:
                    print("REVISED CTC ERROR:", e, raw_value)
                    mapped["Revised_CTC"] = str(raw_value)
                    mapped["Revised_CTC_Words"] = ""
            continue
