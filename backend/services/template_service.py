...
def transform_form_to_mapping_keys(record):
    return {
        k.replace("_", " ").lower(): v for k, v in record.items()
    }


def generate_docx(data, template_path):
    generated = []
    ...
    for record in data:
        ...
        # Normalize keys
        normalized_record = transform_form_to_mapping_keys(record)
        record = transform_form_to_mapping_keys(record)
        mapped_data = map_record(normalized_record)

        # ✅ FIXED DOC NO FROM EXCEL
        mapped_data["DocNo"] = (
            normalized_record.get("docno") or
            normalized_record.get("doc no") or
            normalized_record.get("document number") or
            record.get("DocNo") or
            "DOC001"
        )
